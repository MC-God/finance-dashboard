...
def analyze_portfolio(portfolio_data: str, persona: str) -> str:
    ...
    prompts = {
        ...
        "ten_bagger": (
            ...
            "구글 검색을 통해 최근 주요국의 정부 정책(보조금, 인프라 투자 등)과 핵심 기술 발전이 강력하게 밀어주는 유망 메가트렌드 섹터를 파악하세요. "
            "주어진 포트폴리오 종목 분석에 더해, 해당 트렌드의 수혜를 입어 폭발적 성장이 기대되는 새로운 유망 기업을 하나 발굴하여 추천하십시오. "
            "추천하는 이유와 함께 해당 기업의 간단한 재무 분석(매출 성장세, 이익률, 또는 주요 투자/수주 현황 등)을 포함하여 4~5문장으로 강렬하게 제시하십시오."
        )
    }
